...
def summarize(stats: dict) -> dict:
    ...
    out = {
        ...
        "throughput_tps": {"mean": tps_mean, "std": tps_std},
...
def render_table(config_lines: list[tuple[str, str]], rows: list[dict]) -> str:
    ...
    for r in rows:
        ...
        table.add_row(
            ...
            fmt_count(m["tokens"]),
            f"{m['elapsed_s']:.1f}s",
            fmt_latency(m["latency_ms"]["mean"], m["latency_ms"]["std"], m["batches"]),
            fmt_tps(m["throughput_tps"]["mean"], m["throughput_tps"]["std"], m["batches"]),
        )
